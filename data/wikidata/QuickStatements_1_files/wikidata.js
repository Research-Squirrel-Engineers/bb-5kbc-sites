function WikiDataItem ( init_wd , init_raw ) {
	...
	this.getDatatype = this.getDataytpe ; // alias for typo compatibility
	...
	this.getLabel = function ( language ) {
		let self = this ;
		let item_id = self.getID();
		let label = item_id ; // Fallback
		if ( undefined === language ) {
			$.each ( self.wd.main_languages , function ( dummy , lang ) {
				var l = self.getLabel ( lang ) ;
				if ( l == label ) return ;
		...
		if ( 'mul' != language && label == item_id ) {
			label = self.getLabel('mul'); // Try 'mul' language as fallback
		}
		return label ;
	}
...
/*		if ( claim.rank === undefined ) return undefined ;
	...
	this.bestRank = function ( claims ) {
		let preferred_claims = claims.filter((claim) => this.getClaimRank(claim)=='preferred'); 
		if ( preferred_claims.length>0 ) return preferred_claims;
		let normal_claims = claims.filter((claim) => this.getClaimRank(claim)=='normal'); 
		if ( normal_claims.length>0 ) return normal_claims;
		return claims // Only deprecated left
	}
...
function WikiData () {
	...
	this.sparql_prefixes = ''; // Set this to always prefix SPARQL queries
	...
	this.main_languages = [ 'mul' , 'en' , 'de' , 'fr' , 'nl' , 'es' , 'it' , 'pl' , 'pt' , 'ja' , 'ru' , 'hu' , 'sv' , 'fi' , 'da' , 'cs' , 'sk' , 'et' , 'tr' , 'az' , 'zh' , 'el' ] ;
	this.items = {} ;
	...
	this._cleanApiUrl = function ( url ) {
		return url.replace ( /\?callback=\?$/ , '' ).replace ( /\?callback=\?&/ , '?' ) ;
	}

	this._apiRequest = function ( options ) {
		var self = this ;
		var deferred = $.Deferred() ;
		options.url = self._cleanApiUrl ( options.url ) ;

		function doRequest () {
			var now = Date.now() ;
			var waitTime = Math.max ( 0 , self._nextRequestTime - now ) ;
			setTimeout ( function () {
				$.ajax ( options )
					.done ( function ( data ) {
					...
					.fail ( function ( jqXHR ) {
						if ( jqXHR.status === 429 ) {
							...
							var delay = retryAfter ? parseFloat ( retryAfter ) * 1000 : 5000 ;
							if ( delay < 1000 ) delay = 1000 ;
							self._nextRequestTime = Math.max ( self._nextRequestTime , Date.now() + delay ) ;
		...
		$.each ( ids , function ( dummy , id_list ) {
			...
			self._apiRequest ( {
				...
				$.each ( (data.entities||[]) , function ( k , v ) {
					...
					// console.log("ORIG",q,JSON.parse(JSON.stringify(data.entities[q])));
...
//				languages : self.main_languages.join('|') ,
	...
	this.loadSPARQL = function ( query , callback , callback_fail ) {
		var self = this ;
		let separator = '?';
		if ( this.sparql_url.includes('?') ) separator = '&';
		let final_query = (this.sparql_prefixes+"\n"+query).replace(/\n/g,' ');
		let url = this.sparql_url+separator+"format=json&query=" + encodeURIComponent(final_query) ;
		self._apiRequest ( {
			url : url ,
			dataType : 'json'
		} ) .done ( function ( d ) {
